...
public class KeyboardSwitch {
    public static String ruToEn(String str) {
        return convert(str, KeyboardSwitch::ruToEn); // ссылка на метод перекодировки
    }
    ...
    private static char ruToEn(char c) {
        ...
        char letter = switch (Character.toLowerCase(c)) {
            case 'ф' -> 'a'; case 'и' -> 'b'; case 'с' -> 'c'; case 'в' -> 'd'; case 'у' -> 'e'; case 'а' -> 'f';
            case 'п' -> 'g'; case 'р' -> 'h'; case 'ш' -> 'i'; case 'о' -> 'j'; case 'л' -> 'k'; case 'д' -> 'l';
            case 'ь' -> 'm'; case 'т' -> 'n'; case 'щ' -> 'o'; case 'з' -> 'p'; case 'й' -> 'q'; case 'к' -> 'r';
            case 'ы' -> 's'; case 'е' -> 't'; case 'г' -> 'u'; case 'м' -> 'v'; case 'ц' -> 'w'; case 'ч' -> 'x';
            case 'н' -> 'y'; case 'я' -> 'z';
            default -> c;
        };
